...
export default async function handler(req, res) {
  ...
  const branch = process.env.GITHUB_BRANCH || 'claude/brand-creation-agent-exnb6r';
  ...
  if (!pat || !repo) {
    return res.status(200).json({
      ...
      path,
      content,
      instructions: 'GITHUB_PAT / GITHUB_REPO env vars not set. Copy the content field and commit it manually to the repo at the given path.',
    });
  }
